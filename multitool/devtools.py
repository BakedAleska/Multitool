...
corner (see multitool/ui/layout.py) plus a couple of tools in Settings ->
...
directly; run a packaged build and none of this is present. See
CLAUDE.md's "Developer mode" section for the current feature list -
keep that updated as more lands here.
"""
...
from multitool.logs import LOG_FILE
...
def is_dev_environment() -> bool:
    ...
    A frozen build (see multitool/startup.py, which uses the same
...
def dev_widgets_dir() -> Optional[Path]:
    ...
    Passed straight to multitool.widgets.loader.discover_widgets() as
...
def reload_current_view(page) -> None:
    ...
    multitool.widgets.loader.discover_widgets() always reimports widget
...
def tail_log(lines: int = 300) -> str:
    ...
    needing to go find <DATA_DIR>/logs/multitool.log on disk.
    """
