...
disk as plaintext: ``toolblox.data.crypto`` protects it with DPAPI on
...
from toolblox.data import crypto
...
def save(accounts: list[dict]) -> None:
    ...
    Each record's cookie is protected via ``toolblox.data.crypto`` before
